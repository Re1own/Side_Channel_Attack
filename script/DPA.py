...
def aes_internal(plaintext, key):
    sbox = [
...
# DPA
def DPA_Single_byte(index, textin_array, traces):
    guess_byte = index
    maxs = 0
    ...
    for guess in range(256):
        ...
        maxs = max(maxs, max_diff_value)
        guess_key.append(maxs)
        # print("Guessing {:02x}: {}".format(guess, max_diff_value))
    return np.argmax(guess_key)
...
if __name__=="__main__":
    ...
    guess_key = []
    for i in range(16):
        guess = DPA_Single_byte(i, plaintext, traces)
        print(guess)
        guess_key.append(guess)
    print("guess_key:")
    print(guess_key)
