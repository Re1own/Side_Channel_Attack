...
def get_traces(settings, start, l):
    ...
    traces = np.load("D:\\myhub\\Side_Channel_Attack\\Traces\\Xor_50000_fixed_key\\traces.npy")
    traces = map_to_0_255(traces)
    traces = traces[start:l]
    plaintext = np.load("D:\\myhub\\Side_Channel_Attack\\Traces\\Xor_50000_fixed_key\\p.npy")
    plaintext = plaintext.astype(np.uint16)
    plaintext = plaintext[start:l]
    key =np.load("D:\\myhub\\Side_Channel_Attack\\Traces\\Xor_50000_fixed_key\\k.npy")
